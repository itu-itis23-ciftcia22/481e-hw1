...
\begin{titlepage}
  ...
  \begin{tabular}{ll}
    \textbf{Team Leader:} & Ali Eren Çiftçi - 150220022 \\[0.5em]
    \textbf{Team Member:} & Yavuz Selim Kara - 150220032 \\[0.5em]
  \end{tabular}
...
All experiments were implemented in PyTorch and executed on an Apple Silicon GPU (MPS backend) and an NVIDIA RTX 4060 GPU (CUDA backend).
...
We implement a Convolutional AutoEncoder (CAE) following a symmetric encoder-decoder design. 
Convolutional Neural Networks (CNNs) are selected for this architecture because they are inherently designed for processing 2D grid data like images. Unlike fully connected layers that flatten the input and lose local spatial context, CNNs preserve the spatial arrangements of pixels. They leverage local receptive fields and shared weights to efficiently capture structural visual features such as edges, curves, and shapes.

Furthermore, CNNs perfectly complement the encoder-decoder paradigm. In the encoder pathway, standard convolutional layers progressively reduce the spatial dimensions while increasing the depth of feature maps, distilling the raw image into a compact latent representation. Symmetrically, the decoder employs transposed convolutions to reverse this spatial compression, structurally expanding the condensed latent features back into the original image resolution.

The encoder applies three convolutional blocks to progressively extract and compress spatial features into a fixed-size latent vector. The decoder reconstructs the original image from the latent vector using three transposed convolution blocks.
...
\begin{table}[H]
  ...
  \begin{tabular}{ll}
    ...
    Batch size       & 128 \\
    Epochs           & 15 \\
    Device           & Apple Silicon GPU (MPS) and NVIDIA RTX 4060 \\
...
\begin{figure}[H]
  ...
  % Top Image
  \textbf{Train vs. Test MSE Loss - All Latent Dimensions}\par\vspace{0.5em}
  \includegraphics[scale=0.42]{output_loss_curve1.png}
  
  \vspace{2em} % Adds some breathing room between the top and bottom images
  
  % Bottom Image
  \includegraphics[scale=0.57]{output_loss_curve2.png}
...
\begin{figure}[H]
  ...
  % Top Image Title
  \textbf{Latent Space Visualizations using PCA}\par\vspace{0.5em}
  
  % Top Image
  \includegraphics[width=\textwidth]{output_pca1.png}
  
  \vspace{2em} % Adds breathing room between the top and bottom images
  
  % Bottom Image
  \includegraphics[width=\textwidth]{output_pca2.png}
...
\begin{figure}[H]
  ...
  % Top Image Title
  \textbf{Latent Space Visualization using TSNE}\par\vspace{0.5em}
  
  % Top Image
  \includegraphics[width=\textwidth]{output_tsne1.png}
  
  \vspace{2em} % Adds breathing room between the top and bottom images
  
  % Bottom Image
  \includegraphics[width=\textwidth]{output_tsne2.png}
...
\begin{figure}[H]
  ...
  \vspace{1em} % Adds breathing room between the first and second images
  
  % Second Image
  \includegraphics[scale=0.7]{output2.png}
  
  \vspace{1em} % Adds breathing room between the second and third images
...
\begin{itemize}
  ...
  \item \textbf{Hardware:} Apple Silicon MPS GPU and NVIDIA RTX 4060 GPU
\end{itemize}
